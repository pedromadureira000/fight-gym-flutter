...
class AsyncCustomers extends _$AsyncCustomers {
    ...
    Future<List<Customer>> _fetchData() async {
        ...
        try {
            ...
            if (response.statusCode == 200){
                ...
                // AppConfig.logger.d('customers ${customers}');
                // List<Customer> listCustomers = customers.map(Customer.fromJson).toList();
                ...
                AppConfig.logger.d('totalRecords ${totalRecords}');
                AppConfig.logger.d('listRecords ${listRecords}');
                return listRecords;
            }
    ...
    Future<Customer> getSingleRecord(String? recordId) async {
        try{
            var token = await SecureStorage().readSecureData("token");
            String url = "${AppConfig.backUrl}/customer_get_single_view/$recordId";
            final response = await http.get(
                Uri.parse(url),
                headers: Constants.httpRequestHeaders(token),
            );
